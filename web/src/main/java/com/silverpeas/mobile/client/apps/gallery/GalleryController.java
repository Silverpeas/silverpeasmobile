...
public class GalleryController implements Controller, GalleryControllerEventHandler,
  ...
  public void loadSettings(GalleryLoadSettingsEvent event) {
    ...
    settings.one(new ScalarCallback<GallerySettings>() {
      public void onSuccess(final GallerySettings settings) {
        ServicesLocator.serviceGallery.getAllAlbums(settings.getSelectedGalleryId(),
            new AsyncCallback<List<AlbumDTO>>() {
              @Override
  ...
  public void loadAlbums(final ApplicationInstanceDTO instance) {
    ServicesLocator.serviceGallery.getAllAlbums(instance.getId(),
        new AsyncCallback<List<AlbumDTO>>() {
          @Override
          public void onFailure(Throwable caught) {
            EventBus.getInstance().fireEvent(new ErrorEvent(new Exception(caught)));
                    ...
                    entry.remove(new FileMgrCallback() {
                      ...
                      public void onError(FileError error) {
                        EventBus.getInstance().fireEvent(
                            new ErrorEvent(new Exception(error.toString())));
                        }
                                            });
                    }
              ...
              public void onSuccess(File f) {
                EventCallback callback = new EventCallback() {
                  ...
                  public void onEvent(Event evt) {
                    sendAndRemovePicture(picture, evt.getTarget().getResult(), file, idGallery,
                        idAlbum);
                    }
                                    };
   ...
   * @param idAlbum
   */
  private void sendAndRemovePicture(final Picture picture, String data, final FileEntry file,
      final String idGallery, final String idAlbum) {
    ServicesLocator.serviceGallery.uploadPicture(picture.getName(), data, idGallery, idAlbum,
        ...
        if (count > 0) {
          localsPicturesList.list(new CollectionCallback<Picture>() {
            ...
            public void onSuccess(Picture[] results) {
              ...
              uploadPicture(localsPictures[indexNextPictureToUpload], event.getIdGallery(), event
                  .getIdAlbum());
            }
          });
        }
  ...
  public void loadRemotePictures(RemotePicturesLoadEvent event) {
    ServicesLocator.serviceGallery.getAllPictures(event.getGalleryId(), event.getAlbumId(),
        new AsyncCallback<List<PhotoDTO>>() {
          @Override
  ...
  public void loadRemotePreviewPicture(LoadRemotePreviewPictureEvent event) {
    Notification.activityStart();
    ServicesLocator.serviceGallery.getOriginalPicture(event.getGalleryId(), event.getPhotoId(),
