...
public class VideoServlet extends AbstractSilverpeasMobileServlet {
  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    if (video.canBeAccessedBy(getUserInSession(request))) {
      SilverpeasFile f = video.getFile(MediaResolution.ORIGINAL);
      if (action != null && action.equalsIgnoreCase("download")) {
        // download video
        response.setContentType(((Video) video).getFileMimeType().getMimeType());
        ...
        InputStream in =
            new FileInputStream(MediaHelper.optimizeVideoForWeb(f, request.getSession().getId()));
